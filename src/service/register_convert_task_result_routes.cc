...
namespace Router{
    void ApiRoutes::register_convert_task_result_routes(){
        auto self = shared_from_this();
        drogon::app().registerHandler(
            "/api/v1/products/{product_code}/convert-tasks/{task_id}/result",
            [self](const drogon::HttpRequestPtr &req, std::function<void(const drogon::HttpResponsePtr &)> &&callback, const std::string &product_code, const std::string &task_id){
                Json json = self->m_convertTaskAction.getTaskResult(task_id);
